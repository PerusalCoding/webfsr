...
class Sensor {
 public:
  ...
  void EvaluateSensor(bool willSend) {
    ...
    //
    // Uses micros() rather than millis() so the debounce window is
    // accurate even when the loop runs faster than 1ms -- millis() has
    // only 1ms resolution and can make a 15ms debounce fire as early as
    // 14ms depending on where in a millisecond tick the timer started.
    // Ported over from the private/dev build.
    if (below_release_now) {
      if (!was_below_release_) {
        below_release_since_us_ = micros();
    ...
    bool should_be_off = was_below_release_ &&
        ((micros() - below_release_since_us_) >= (unsigned long)release_debounce_ms_ * 1000UL);
  ...
  void UpdateThreshold(int16_t t) {
    ...
    release_auto_ = true;  // legacy single-value path is always auto-follow
  }

  // New explicit dual-threshold API.
  //
  // release_auto_ starts true, meaning release_threshold_ tracks
  // trigger_threshold_ (trigger - kDefaultPaddingWidth) automatically --
  // this matches "Sensor Tuning" being off in the dashboard, where the
  // user only ever touches Trigger and expects Release (and therefore
  // the LEDs, which key off the same is_active_ state) to just follow
  // it. The moment SetReleaseThreshold() is called explicitly (i.e. the
  // user turns Sensor Tuning on and sets their own release value),
  // release_auto_ flips false and this sensor's release value stays
  // exactly where they put it -- future Trigger-only changes no longer
  // drag it around.
  void SetTriggerThreshold(int16_t t) {
    trigger_threshold_ = constrain(t, 0, 1023);
    if (release_auto_) {
      int16_t derived = trigger_threshold_ - kDefaultPaddingWidth;
      release_threshold_ = (derived < 0) ? 0 : derived;
    }
  }
  void SetReleaseThreshold(int16_t t) {
    release_threshold_ = constrain(t, 0, 1023);
    release_auto_ = false;  // user has now manually tuned release
  }
 ...
 private:
  ...
  // True until SetReleaseThreshold() is called explicitly for this
  // sensor. See SetTriggerThreshold()/SetReleaseThreshold() above.
  bool    release_auto_ = true;
  ...
  // below_release_since_us_/was_below_release_ track how long the
  // current below-threshold streak has lasted.
  uint16_t release_debounce_ms_;
  unsigned long below_release_since_us_;   // micros() timestamp of when below-release streak started
...
// public release.
const char* const kFirmwareVersion = "1.0.";
