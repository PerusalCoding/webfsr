...
const char* const kFirmwareVersion = "1.0.0";
...
void SaveSensorConfigToEeprom() {
  EEPROM.write(0, kEepromMarker);
  EEPROM.write(1, (uint8_t)kNumSensors);
  ...
  for (size_t i = 0; i < MAX_SENSORS; ++i) {
    ...
    EEPROM.write(base + 0, offset);
    EEPROM.write(base + 1, count);
    EEPROM.write(base + 2, c.r);
    EEPROM.write(base + 3, c.g);
    EEPROM.write(base + 4, c.b);
    EEPROM.write(base + 5, trig & 0xFF);
    EEPROM.write(base + 6, trig >> 8);
    EEPROM.write(base + 7, rel & 0xFF);
    EEPROM.write(base + 8, rel >> 8);
    EEPROM.write(base + 9, gain & 0xFF);
    EEPROM.write(base + 10, gain >> 8);
    EEPROM.write(base + 11, group);
    EEPROM.write(base + 12, debounce);
  }
}
...
class EepromProcessor {
  ...
  private:
    ...
    void SaveThreshold(int slot, size_t idx, int16_t val) {
      ...
      EEPROM.write(off + idx*2,   val & 0xFF);
      EEPROM.write(off + idx*2+1, val >> 8);
    }
...
// Format: "i <version> <eeprom_schema_hex> <num_sensors>"
// Example: "i 1.0.0 A8 4"
